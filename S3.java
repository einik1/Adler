...
public class S3 {
	...
	public S3(String directoryName){
		 this.directoryName = directoryName;
		 this.bucketName = credentialsProvider.getCredentials().getAWSAccessKeyId();
		 this.bucketName = this.bucketName.toLowerCase();
		 this.key = null;
	}
	...
	public void uploadFileToBucket(){
        ...
        try {
            ...
            for (File file : dir.listFiles()) {
                ...
                PutObjectRequest req = new PutObjectRequest(this.bucketName, this.key, file);
                s3.putObject(req);
            }
        } catch (AmazonServiceException ase) {
            System.out.println("Caught an AmazonServiceException, which means your request made it "
	...
	public void dowlaodObjectFromBucket() throws IOException{
	try {
	...
	public void deleteFileFromBucket(){
         ...
         */
      	try {
		     ...
		     s3.deleteObject(this.bucketName, this.key);          
		     System.out.println();
	} catch (AmazonServiceException ase) {
        System.out.println("Caught an AmazonServiceException, which means your request made it "
                + "to Amazon S3, but was rejected with an error response for some reason.");
	...
	public void deleteBucket(){
      ...
      */
		try {
		  ...
		  s3.deleteBucket(this.bucketName); 
		} catch (AmazonServiceException ase) {
            System.out.println("Caught an AmazonServiceException, which means your request made it "
                    + "to Amazon S3, but was rejected with an error response for some reason.");
            System.out.println("Error Message:    " + ase.getMessage());
